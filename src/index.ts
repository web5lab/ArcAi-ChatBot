...
export function Chatbot(config: ChatbotConfig) {
  ...
  let isOpen = false;
  let thoughtTimeout: string | number | NodeJS.Timeout | undefined;
  let messages = [];
  ...
  function injectStyles(config: any) {
  ...
  function renderMessage(message: { id?: string; role: any; content: any; timestamp?: Date; }) {
  ...
  function showThought() {
    const thought = chatbotConfig.customThoughts[
      Math.floor(Math.random() * chatbotConfig.customThoughts.length)
    ];
  ...
  async function defaultMessageHandler(message: any) {
   return message
  }
...
function sendTransactionFunction(to: any, amount: any) {
